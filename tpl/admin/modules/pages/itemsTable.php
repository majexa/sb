  ...
  id="<?= 'item_'.$v['id'] ?>">
    ...
    <td class="tools loader">
      ...
      <a class="iconBtn <?= $v['active'] ? 'activate' : 'deactivate' ?>" title="<?= $v['active'] ? LANG_HIDE : LANG_SHOW ?>"
        href="<?= $this->getPath(3) ?>?a=<?= ($v['active'] ? 'deactivate' : 'activate') . '&id='.$v['id'] ?>"><i></i></a>
      <?php /*
      <a class="iconBtn move" title="<?= LANG_MOVE ?>"
        href="<?= $this->getPath(3) ?>?a=moveForm&id=<?= $v['id'] ?>"><i></i></a>
      <? if ($v['editebleContent']) { ?>        
        <a class="iconBtn <?= $d['favorits'][$v['id']] ? 'favorit' : 'favoritOff'?>" title="<?= LANG_ADD_ANCHOR ?>"
          href="<?= $this->getPath(3).'/'.($d['favorits'][$v['id']] ? '?a=deleteFavorit&id='.$v['id'] : '?a=addFavorit') ?>"><i></i></a>
      <? } else { ?>
        <div class="dummyIconBtn"></div>
      <? } ?>
      */?>
      <a class="iconBtn <?= $v['onMenu'] ? 'sitemap' : 'sitemapRed' ?>" title="<?= $v['onMenu'] ? LANG_SHOW_IN_MENU : LANG_NOT_SHOW_IN_MENU ?>"
        href="<?= $this->getPath(3) ?>?a=onMenu&onMenu=<?= $v['onMenu'] ? 0 : 1 ?>&id=<?= $v['id'] ?>"><i></i></a>
      <? } ?>
      <div class="clear"><!-- --></div>
    </td>
      <?php
    ...
    <td class="tools title"<?= $v['isLock'] ? ' title="<?= LANG_ORANGE_FLAG ?>"' : '' ?>>
      <div class="<?= $itemIconClass ?>">
        <? if ($v['folder']) { ?>
          <a href="<?= $this->getPath(2).'/'.$v['id'] ?>" class="tooltip" title="Открыть папку">
          <i></i><?= $v['title'] ?></a>
        <? } elseif ($v['editebleContent']) { ?>
          <i></i><a href="<?= $this->getPath(2).'/'.$v['id'] ?>/editContent" class="tooltip" title="Редактировать содержание"><?= $v['title'] ?></a>
        <? } else { ?>
          <i></i><?= $v['title'] ?>
        <? } ?>
      </div>
    ...
    <td>
      <a href="<?= $this->getPath(0).'/'.$v['path'] ?>" target="_blank" class="smIcons link gray"
        title="<?= LANG_GO_TO_PAGE ?>"><i></i>
      <small><?= Misc::cut($v['path'], 20) ?></small></a>
      <? if ($v['link']) { ?>
        <div style="float:left; margin-right:10px; color: #999999">→</div>
        <a href="<?= $v['link'] ?>" target="_blank" class="smIcons link gray"
