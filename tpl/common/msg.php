...
$d['userPath'] = $this->getUserPath($v['userId'], true);
...
<div class="item<?= $v['active'] ? '' : ' nonActive' ?><?= ($v['userId'] == Auth::get('id') ? ' owner' : '') ?><?= $d['newMsgClass'] ?><?= $d['userPath'] ? '' : ' noAvatars' ?>" id="msg<?= $v['id'] ?>">
  ...
  <div class="textd">
    <?
    if ($d['canEdit']) $this->tpl('editBlocks/editBlockSubAjax', $v);
    ?>
    <div class="author">
      <? if ($v['userId']) { ?>
        <? if ($v['login']) { ?>
      ...
      <? } ?>
      <? if ($v['ansLogin']) { ?>
        → <a href="<?= $this->getUserPath($v['ansUserId']) ?>"><i></i><?= $v['ansLogin'] ?></a>
      <? } ?>
    </div>
    <div class="text" id="msgText<?= $v['id'] ?>"><?= $v['text_f'] ?></div>
    <div class="subscript">
      <a href="<?= $this->getPath() ?>#cmt<?= $v['id'] ?>" title="Ссылка на этот пост" class="gray anchor">
      <small><?= datetimeStr($v['dateCreate_tStamp']) ?></small></a>
      <? if ($v['canCreate'] and $v['login'] and Auth::get('id') != $v['userId']) { ?>
        <a href="#" class="smIcons answer gray" title="Ответить"><i></i>ответить</a>
      <? } ?>
      <? if ($v['showPage']) { ?>
        <p class="gray">Раздел:
  ...
  <div class="data" style="display:none"><?= json_encode([
    'id' => $v['id'],
    'userId' => $v['userId'],
