...
Ngn.site.top.Auth = new Class({
  ...
  initialize: function(options) {
    ...
    if (eTopSlideTip) {
      var st = (eTopSlideTip.get('class') == 'topSlideTipPm' && this.options.privMsgsPath) ? this.getPmSlideTips(eTopSlideTip) : new Ngn.SlideTips(eTopSlideTip);
      st.setOptions({
        onShow: function() {
          eTopSlideTip.setStyle('left', -Math.round(this.element.getSize().x / 2 - this.element.getPrevious().getSize().x / 2));
        }
      });
    }
  ...
  getPmSlideTips: function(eTopSlideTip) {
    return new Ngn.SlideTips.Pm(eTopSlideTip, {
