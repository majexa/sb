...
Ngn.Sb.DdItems = new Class({
  ...
  addEditBlock: function(eItem) {
    ...
    var eEditBlock = Elements.from(Ngn.strReplace('{path}', this.options.editPath, Ngn.strReplace('{id}', eItem.get(idParam), this.editBlockTpl)))[0];
    ...
    var btnDelete = eEditBlock.getElement('.delete');
    ...
    var btnActivate = eEditBlock.getElement('.activate');
    btnActivate.removeClass('activate').addClass(active ? 'deactivate' : 'activate');
        ...
        new Ngn.Request({
          ...
          onComplete: function() {
            ...
            if (active) {
              ...
              btnActivate.removeClass('deactivate').addClass('activate');
              btnActivate.set('title', 'Показать');
            } else {
              active = true;
              eItem.removeClass('nonActive');
              btnActivate.removeClass('activate').addClass('deactivate');
...
Ngn.Sb.DdItems.Dialog = new Class({
...
Ngn.Sb.DdItems.next = function(esItems, curId) {
  return Ngn.Sb.DdItems._next(esItems, curId, true);
};

Ngn.Sb.DdItems.prev = function(esItems, curId) {
  return Ngn.Sb.DdItems._next(esItems, curId, false);
};
