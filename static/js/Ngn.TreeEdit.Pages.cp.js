Ngn.TreeEdit.Pages.cp = new Class({
  ...
  bindButtons: function() {
    ...
    this.createButton('controllerSettings',
      this.eButtons.getElement('a[class~=controllerSettings]'),
      function() {
        this.twoScreens.openControllerSettingsDialog(this.tree.selected.data.id);
      }.bind(this)
    );

    this.createButton('slaveControllerSettings',
      this.eButtons.getElement('a[class~=slaveControllerSettings]'),
      function() {
    ...
    this.createButton('ddo', this.eButtons.getElement('a[class~=ddo]'), function() {
      window.location = Ngn.getPath(1) + '/ddo/' + this.tree.selected.data.id;
    }.bind(this));
    ...
    this.createButton('onMap', this.eButtons.getElement('a[class~=onMap]'), function() {
      new Ngn.Request({
        url: Ngn.getPath(1) + '/pages/' + this.tree.selected.data.id + '?a=ajax_onMenu&onMenu=' +
          (this.tree.selected.data.onMap ? 0 : 1),
        onComplete: function() {
          this.tree.reload();
