Ngn.TreeEdit.Pages.Cp = new Class({
  ...
  bindButtons: function() {
    ...
    this.createButton('editContent', this.eButtons.getElement('a[class~=edit]'), function() {
      window.location = Ngn.getPath(1) + '/pages/' + this.tree.selected.data.id + '/editContent';
    }.bind(this));
    ...
    this.createButton('onMap', this.eButtons.getElement('a[class~=onMap]'), function() {
      new Ngn.Request({
        url: Ngn.getPath(1) + '/pages/' + this.tree.selected.data.id + '?a=ajax_onMenu&onMenu=' + (this.tree.selected.data.onMap ? 0 : 1),
        onComplete: function() {
          this.tree.reload();
