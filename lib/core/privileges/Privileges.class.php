...
class Privileges {
  ...
  static function check($userId, $pageId, $type) {
    $r = db()->selectRow(
  ...
  static function extendItemPriv(&$item, $userId, $seconds) {
    ...
    if (!isset($item['dateCreate_tStamp']))
      throw new Exception("\$item['dateCreate_tStamp'] not defined. \$item: ".getPrr($item));
