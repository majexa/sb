...
class PageModuleTreeImporter {
  ...
  function __construct() {
    ...
    if (($page = DbModelPages::getHomepage()) !== false) $this->defaultParentId = $page['id'];
  }

  protected function createHomepage() {
    if ($this->createHomepageIfNotExists and empty($this->defaultParentId)) {
      $this->defaultParentId = DbModelCore::create('pages', [
        'title'  => 'Главная',
        'name'   => 'main',
        'active' => 1,
        'folder' => 1,
  ...
  function importNodes(array $nodes) {
    $this->ids = [];
    $this->createHomepage();
    foreach ($nodes as $node) $this->processNode2($node);
  }
  ...
  function processNode($v) {
    if (!isset($v['n'])) $v['n'] = 10;
    Arr::checkEmpty($v, ['title', 'module']);
    if (isset($v['parent']) and isset($this->ids[$v['parent']])) $v['parentId'] = $this->ids[$v['parent']];
    elseif ($this->defaultParentId) $v['parentId'] = $this->defaultParentId;
    elseif (empty($v['parentId'])) $v['parentId'] = 0;
  ...
  function processNode2($v) {
    Arr::checkIsset($v, ['title', 'parent']);
  ...
  private function getModuleController($name) {
    if (!isset($this->modules[$name])) throw new Exception("Module Package Module '$name' does not exists");
