...
class PagesAdmin extends PagesTree {
  ...
  private function updateSettings($id, $initSettings, $isAdd = true) {
    if (!($page = $this->getNode($id)))
      throw new Exception("Page ID=$id does not exists");
  ...
  function updateSettingsValue($id, $k, $v) {
  ...
  function beforeUpdateAction($id, &$data) {
    $data['id'] = $id;
    return;
