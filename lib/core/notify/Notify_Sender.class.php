...
class Notify_Sender {
   ...
   * @return  bool
   */
  function send($fromUserId, $toUserId, $sendMethods, $title, $text) {
    $n = 0;
    // Если у этого пользователя есть такой метод подписки
    $userSendMethods = UserSettings::get($toUserId, 'sendMethods');
  ...
  function sendIfSubscribed($subscribeType, $fromUserId, $toUserId, $sendMethods, $title, $text) {
  ...
  function send_privMsgs($fromUserId, $toUserId, $title, $text) {
  ...
  function send_email($fromUserId, $toUserId, $title, $text) {
    ...
    return (new SendEmail())->send($toUserData['email'], $title, $text);
  }
