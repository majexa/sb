...
class CtrlCommonRoundCorners extends CtrlCommon {
  ...
  protected function init() {
    ...
    $this->w = !empty($this->req->params[6]) ? $this->req->params[6] : 500;
    $this->h = !empty($this->req->params[7]) ? $this->req->params[7] : 500;
    $this->bgColor = '#'.$this->req->param(3);
    $this->borderColor = '#'.$this->req->param(4);
    if (getOS() != 'win') {
      $this->bgColor = "'$this->bgColor'";
      $this->borderColor = "'$this->borderColor'";
    }
    $this->radius = $this->req->param(5);
