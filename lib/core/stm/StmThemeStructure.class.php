...
class StmThemeStructure extends Options2 {
  ...
  static $requiredOptions = ['siteSet', 'design'];

  public $str;

  protected function init() {
    $file = STM_DESIGN_PATH.'/'.$this->options['siteSet'].'/'.$this->options['design'].'/structure.php';
    if (!file_exists($file)) throw new NoFileException($file);
...
-
  ...
  s: ".contentBody img, .pageBlocks .blockHeader, .items .item, #calendarHeader, .bordered.smIcons a:hover, a:hover.smIcons.bordered, .apeform .element, .pageBlocks .pbt_tags a, .avatar a, .thumb a, a.thumb, .pageBlocks .btitle"
  p: border-color
  type: color
-
...
-
  ...
  type: pixels
-
  title: Отступ слева от логотипа
  s: "#logo"
  p: margin-left
  type: pixels
-
...
-
  ...
  type: select
  options:
    default: Текст по центру
    2colsCounters: 2 колонки со счетчиками
-
  title: Отображать панель авторизации
  name: topEnable
  type: bool
# =====================================================
-
