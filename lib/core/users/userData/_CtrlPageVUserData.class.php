...
class CtrlPageVUserData extends CtrlPage {
  ...
  public $requiredSettings = ['profiles'];
  ...
  protected function initPathAndTitle() {
    $this->setPathDataLast(Tt()->getPath(), $this->d['user']['login']);
    if (empty($this->d['user']['login']))
      throw new Exception('User ID='.$this->d['user']['id'].' has empty login');
