...
class StoreOrderForm extends Form {
  ...
  function __construct(array $cartItems, array $options = []) {
    ...
    parent::__construct(
      new Fields(),
      array_merge(['submitTitle' => 'Отправить заказ'], $options)
    );
  }
  ...
  protected function init() {
    if (Auth::get('id')) {
      ...
      $this->fields->fields = array_merge(
        $this->im->form->fields->fields,
  ...
  protected function _update(array $data) {
    ...
    if (($behaviors = Config::getVarVar('store', 'orderBehaviors', true))) {
      foreach ($behaviors as $v)
        Ngn::fireEvent("store.newOrder.$v", [
          $orderId,
          $this->cartItems,
        ]);
    }
